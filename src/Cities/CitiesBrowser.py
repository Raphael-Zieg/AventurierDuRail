...
class CitiesSelection:
    ...
    def get_cities_df(
        ...
        worldcities = worldcities.applymap(lambda x: x.replace('"', "") if isinstance(x, str) else x)

        merged_df = pd.merge(worldcities, continents_country, left_on="iso3", right_on="Code")

        result_df = merged_df[["city_ascii", "lat", "lng", "population", "Entity", "Continent"]]
        ...
        top_cities_by_country = result_df.groupby("Entity").apply(top_N_cities).reset_index(drop=True)
    ...
    def _compute_distances(self, df):
        ...
        for i in range(num_cities):
            distance_matrix[i, :] = self.__haversine(df["lat"].iloc[i], df["lng"].iloc[i], df["lat"].values, df["lng"].values)

        # Créer un DataFrame à partir de la matrice des distances pour l'affichage ou l'analyse ultérieure
        return pd.DataFrame(distance_matrix, index=df["city"], columns=df["city"], dtype="int")

    def __haversine(self, lat1, lon1, lat2, lon2):
        """Calcul la distance entre deux points (latitude,longitude) avec la formule de haversine
        Args : vecteurs latitude, longitude
        Return : vecteur distance en km
        """
